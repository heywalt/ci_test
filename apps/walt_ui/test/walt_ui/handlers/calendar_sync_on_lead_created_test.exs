defmodule WaltUi.Handlers.CalendarSyncOnLeadCreatedTest do
  ...
  alias WaltUi.Handlers.CalendarSyncOnLeadCreated
  ...
  describe "handle/2" do
    test "enqueues calendar sync job when lead created with email field" do
      Oban.Testing.with_testing_mode(:manual, fn ->
        ...
        assert :ok = CalendarSyncOnLeadCreated.handle(event, metadata)

        assert_enqueued(
          worker: SyncContactCalendarEventsJob,
          args: %{
    ...
    test "enqueues calendar sync job when lead created with emails field" do
      Oban.Testing.with_testing_mode(:manual, fn ->
        user_id = Ecto.UUID.generate()
        contact_id = Ecto.UUID.generate()
        external_account_id = Ecto.UUID.generate()
    ...
    test "does not enqueue job when lead created without email fields" do
      ...
      refute_enqueued(worker: SyncContactCalendarEventsJob)
    end
